...
void transpose_matrix(int **matrix, int row, int colunm);                                            // transpose_matrix 함수의 원형 선언
void multiply_matrix(int **matrixA, int rowA, int colunmA, int **matrixB, int rowB, int colunmB);    // multiply_matrix 함수의 원형 선언
...
int main()
{
    ...
    if (rowA == rowB && columnA == columnB) // rowA == rowB 이고 columnA == columnB 이면
    {
    ...
    transpose_matrix(matrixA, rowA, columnA); // matrixA, rowA, columnA를 매개변수로 받는 함수 transpose_matrix 실행
    transpose_matrix(matrixB, rowB, columnB); // matrixB, rowB, columnB를 매개변수로 받는 함수 transpose_matrix 실행

    if (columnA == rowB) // columnA == rowB 이면
    {
        multiply_matrix(matrixA, rowA, columnA, matrixB, rowB, columnB); // matrixA, rowA, columnA, matrixB, rowB, columnB를 매개변수로 받는 함수 multiply_matrix 실행
    }
    else
    {
        printf("columnA != rowB\n");
...
// 행렬을 화면에 출력하는 함수
{
    ...
    {
        for (j = 0; j < colunm; j++) // j를 0부터 1씩 증가시키며 colunm보다 작을 때까지 반복
        {
            printf("%5d", matrix[i][j]); // matrix[i][j] 값 출력
        }
        printf("\n");
    }
}
...
// int형 이중 포인터 변수 matrixA, matrixB int형 변수 rowA, colunmA, rowB, colunmB를 매개변수로 갖는 void형 함수 addition_matrix
{
    ...
    for (i = 0; i < rowA; i++)                             // i를 0부터 1씩 증가시키며 rowA보다 작을 때까지 반복
        matrixC[i] = (int *)malloc(sizeof(int) * colunmA); // matrixC[i]에 (int의 크기 * colunmA)만큼 메모리 할당

    for (i = 0; i < rowA; i++) // i를 0부터 1씩 증가시키며 rowA보다 작을 때까지 반복
    {
        for (j = 0; j < colunmA; j++)                      // j를 0부터 1씩 증가시키며 colunmA보다 작을 때까지 반복
            matrixC[i][j] = matrixA[i][j] + matrixB[i][j]; // matrixC[i][j]에 matrixA[i][j] + matrixB[i][j] 값을 대입
    }
...
// int형 이중 포인터 변수 matrix, int형 변수 row를 매개변수로 갖는 void형 함수 free_matrix
{
    for (i = 0; i < row; i++) // i를 0부터 1씩 증가시키며 row보다 작을 때까지 반복
        free(matrix[i]);      // matrix[i]에 할당되었던 메모리 해제
    free(matrix);             // matrix에 할당되었던 메모리 해제
}
void transpose_matrix(int **matrix, int row, int colunm)
// int형 이중 포인터 변수 matrix, int형 변수 row, colunm를 매개변수로 갖는 void형 함수 transpose_matrix
{
    int **matrixC;                                     // int형 이중 포인터 matrixC 선언
    matrixC = (int **)malloc(sizeof(int *) * colunm);  // matrixC에 (포인터의 크기 * colunm)만큼 메모리 할당
    for (i = 0; i < colunm; i++)                       // i를 0부터 1씩 증가시키며 colunm보다 작을 때까지 반복
        matrixC[i] = (int *)malloc(sizeof(int) * row); // matrixC[i]에 (int의 크기 * row)만큼 메모리 할당

    for (i = 0; i < colunm; i++) // i를 0부터 1씩 증가시키며 colunm보다 작을 때까지 반복
    {
        for (j = 0; j < row; j++)         // j를 0부터 1씩 증가시키며 row보다 작을 때까지 반복
            matrixC[i][j] = matrix[j][i]; // matrixC[i][j]에 matrix[j][i] 값을 대입
    }

    printf("\n----------Transposition Results----------\n\n");
    print_matrix(matrixC, colunm, row); // matrixC, column, row를 매개변수로 받는 함수 print_matrix 실행

    free_matrix(matrixC, colunm); // matrixC, colunm를 매개변수로 받는 함수 free_matrix 실행
}
void multiply_matrix(int **matrixA, int rowA, int colunmA, int **matrixB, int rowB, int colunmB)
// int형 이중 포인터 변수 matrixA, matrixB int형 변수 rowA, colunmA, rowB, colunmB를 매개변수로 갖는 void형 함수 multiply_matrix
{
    int **matrixC;                                         // int형 이중 포인터 matrixC 선언
    matrixC = (int **)malloc(sizeof(int *) * rowA);        // matrixC에 (포인터의 크기 * rowA)만큼 메모리 할당
    for (i = 0; i < rowA; i++)                             // i를 0부터 1씩 증가시키며 rowA보다 작을 때까지 반복
        matrixC[i] = (int *)malloc(sizeof(int) * colunmB); // matrix[i]에 (int의 크기 * colunmB)만큼 메모리 할당

    for (i = 0; i < rowA; i++) // i를 0부터 1씩 증가시키며 rowA보다 작을 때까지 반복
    {
        for (j = 0; j < colunmB; j++) // j를 0부터 1씩 증가시키며 colunmB보다 작을 때까지 반복
        {
            matrixC[i][j] = 0; // matrixC[i][j]에 0 대입
        }
    }
    ...
    for (i = 0; i < rowA; i++) // i를 0부터 1씩 증가시키며 rowA보다 작을 때까지 반복
    {
        for (j = 0; j < colunmB; j++) // j를 0부터 1씩 증가시키며 colunmB보다 작을 때까지 반복
        {
            for (k = 0; k < colunmA; k++) // k를 0부터 1씩 증가시키며 colunmA보다 작을 때까지 반복
            {
                matrixC[i][j] += matrixA[i][k] * matrixB[k][j]; // matrixC[i][j]에 matrixC[i][j] + (matrixA[i][k] * matrixB[k][j]) 값을 대입
            }
        }
    }

    printf("\n----------Multiplication Results----------\n\n");
    print_matrix(matrixC, rowA, colunmB); // matrixC, rowA, columnB를 매개변수로 받는 함수 print_matrix 실행
